...
const updateUserEntry = async (best_pa, test) => {
  try {
      ...
      const response = await fetch('/update-entry/', {
          ...
          body: JSON.stringify({ best_pa, test })
      });
...
function displayScore(){
  ...
  test_value++
  if(percentageAcc>best_pa_value){
    best_pa_value=percentageAcc;
  }
  // store to db 
  updateUserEntry(best_pa_value,test_value);
